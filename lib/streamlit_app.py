...
from lib import bom as bom_lib
from lib import branding, bundle
from lib import invoice as invoice_lib
from lib import parsers, passwords, vision
...
def _sales_import(snap: Snapshot, data: dict, actor: User) -> None:
    ...
    if frame is not None and not frame.empty:
        ...
        keys = ("order", "name", "option", "qty", "status", "date",
                "product_id", "option_id")
        guessed = {k: parsers.guess_column(columns, k) for k in keys}
        ...
        picked = {
            ...
            "product_id": pick("상품번호 열", "product_id", col_a),
            "option_id": pick("옵션ID 열", "option_id", col_b),
            "status": pick("주문상태 열", "status", col_a),
            "date": pick("주문일자 열", "date", col_b),
        }
        st.caption(
            "상품번호와 옵션ID가 있으면 꼭 골라주세요. 같은 상품이라도 옵션마다 "
            "구성이 다를 수 있어, 이름보다 훨씬 정확합니다."
        )
        ...
        if st.button("확인하기"):
            if not picked["qty"] or not (picked["name"] or picked["option"]):
                ...
                resolve = bom_lib.build_index(get_store().read_boms(),
                                              data["channel_mapping"])
                st.session_state.sale_preview = parsers.parse_sales(
                    frame, channel, picked, resolve,
                    get_store().existing_txn_keys(), today_kst())
    ...
    st.markdown("## 확인")
    st.write(
        f"반영 {counts.get('반영', 0)}건 · 취소복원 {counts.get('취소복원', 0)}건 · "
        f"중복 {counts.get('중복', 0)}건 · 신규구성 {counts.get('신규구성', 0)}건"
    )

    table = []
    for _, row in preview.iterrows():
        detail = " / ".join(
            f"{names.get(c, c)} {q:+d}" for c, q in row["components"]) or "—"
        table.append({
            "판매상품": row["raw_name"][:36],
            "주문": row["order_qty"],
            "실제 재고차감": detail,
            "상태": row["status"],
            "근거": row["why"],
    ...
    _new_bundles(preview, snap, data, actor, channel, names)

    applicable = preview[preview["status"].isin(["반영", "취소복원"])]
    if applicable.empty:
        return

    total = sum(row["deduct"] for _, row in applicable.iterrows())
    if not st.button(f"{len(applicable)}건 장부에 반영 (실제 {total:,}개)", type="primary"):
        return

    entries = []
    for _, row in applicable.iterrows():
        restoring = row["status"] == "취소복원"
        for (code, qty), key in zip(row["components"], row["txn_keys"]):
            entries.append({
                "txn_key": key, "product_code": code, "qty_change": qty,
                "reason": "CANCEL" if restoring else channel,
    ...
    summary = audit_lib.entry(
        ...
        qty_change=-total,
        note=f"{len(applicable)}건 · 실제 {total}개 · "
             f"중복 {counts.get('중복', 0)}건 · 신규구성 {counts.get('신규구성', 0)}건",
    )
    result = write_ledger(entries, actor, [summary])
    st.session_state.sale_preview = None
    st.success(f"{result['written']}줄 기록했습니다.")
    st.rerun()


def _new_bundles(preview, snap: Snapshot, data: dict, actor: User,
                 channel: str, names: dict) -> None:
    """
    처음 보는 판매상품의 구성을 정한다.

    구성이 정해지기 전에는 그 주문을 재고에 반영하지 않는다.
    상품명만 보고 짐작해 차감하면, 어느 재고가 왜 줄었는지 알 수 없게 된다.
    """
    fresh = preview[preview["status"] == "신규구성"]
    if fresh.empty:
        return

    st.markdown("## 처음 보는 판매상품")
    st.caption(
        "구성을 정해야 재고에 반영됩니다. 한 번 저장하면 다음 판매 파일부터는 "
        "자동으로 분해됩니다."
    )
    ...
    for _, row in fresh.iterrows():
        key = (row["product_id"], row["option_id"], row["raw_name"])
        if key in seen:
            continue
        seen.add(key)
        _bundle_form(row, snap, data, actor, channel, labels, choices, by_label)


def _bundle_form(row, snap: Snapshot, data: dict, actor: User, channel: str,
                 labels: dict, choices: list, by_label: dict) -> None:
    slug = re.sub(r"\W", "", f"{row['product_id']}{row['option_id']}{row['raw_name']}")[:40]
    state_key = f"bundle_{slug}"

    with st.expander(f"{row['raw_name'][:44]}  (주문 {row['order_qty']}개)"):
        if row["product_id"]:
            st.caption(f"상품번호 {row['product_id']}"
                       + (f" · 옵션ID {row['option_id']}" if row["option_id"] else ""))
        else:
            st.caption("상품번호가 없어 상품명으로 저장됩니다. "
                       "다음부터는 판매 파일에서 상품번호 열을 골라주세요.")

        col_a, col_b = st.columns(2)
        if col_a.button("AI에게 구성 물어보기", key=f"ask_{slug}"):
            api_key = st.secrets.get("anthropic_api_key", "")
            try:
                with st.spinner("구성을 분석하고 있습니다"):
                    proposed, confidence, note = bundle.propose(
                        channel, row["name"], row["option"], data["product"],
                        api_key, st.secrets.get("claude_model", bundle.DEFAULT_MODEL))
                st.session_state[state_key] = proposed
                st.session_state[f"{state_key}_note"] = note
                st.session_state[f"{state_key}_conf"] = confidence
            except ValueError as exc:
                st.warning(str(exc))
                offline = bundle.guess_offline(row["name"], row["option"], data["product"])
                if offline:
                    st.session_state[state_key] = offline
                    st.session_state[f"{state_key}_note"] = \
                        "상품명에 이름이 들어 있는 상품만 찾았습니다. 수량은 직접 확인하세요."
            st.rerun()

        if col_b.button("직접 고르기", key=f"manual_{slug}"):
            st.session_state[state_key] = bom_lib.Bom(channel=channel, components=[])
            st.rerun()

        draft = st.session_state.get(state_key)
        if draft is None:
            st.info("AI에게 물어보거나 직접 고르세요.")
            return

        note = st.session_state.get(f"{state_key}_note", "")
        if note:
            st.warning(note)
        if st.session_state.get(f"{state_key}_conf") == "low":
            st.warning("확신이 낮은 제안입니다. 구성품과 수량을 꼭 확인하세요.")

        st.caption("판매 1개당 빠지는 개수를 적으세요. 주문 수량은 곱하지 않습니다.")
        rows = [{"구성품": labels.get(c.product_code, c.product_code), "개수": c.qty}
                for c in draft.components] or [{"구성품": choices[0] if choices else "", "개수": 1}]

        edited = st.data_editor(
            pd.DataFrame(rows), hide_index=True, use_container_width=True,
            num_rows="dynamic",
            column_config={
                "구성품": st.column_config.SelectboxColumn(options=choices, width="medium"),
                "개수": st.column_config.NumberColumn(min_value=1, step=1, width="small"),
            },
            key=f"editor_{slug}",
        )

        components = []
        for _, line in edited.iterrows():
            code = by_label.get(line["구성품"])
            qty = int(line["개수"] or 0)
            if code and qty > 0:
                components.append(bom_lib.Component(code, qty))

        if not components:
            st.caption("구성품을 하나 이상 골라야 저장할 수 있습니다.")
            return

        preview_bom = bom_lib.Bom(channel=channel, components=components)
        st.write(f"**{row['order_qty']}개 주문 시 실제 차감** — " + " / ".join(
            f"{labels.get(c, c)} {q}개" for c, q in preview_bom.expand(row["order_qty"])))

        if st.button("구성 저장", key=f"save_{slug}", type="primary"):
            saving = bom_lib.Bom(
                channel=channel,
                channel_product_id=row["product_id"], channel_option_id=row["option_id"],
                channel_name=row["name"], option_name=row["option"],
                components=components,
            )
            store = get_store()
            store.add_bom_rows(bom_lib.to_rows(saving, actor.login_id), actor.login_id)
            store.append_audit([audit_lib.entry(
                actor, "BOM_CREATE", feature="판매구성 등록",
                after=bom_lib.describe(saving, labels),
                ref_no=row["product_id"] or "", note=row["raw_name"],
            )])
            for suffix in ("", "_note", "_conf"):
                st.session_state.pop(f"{state_key}{suffix}", None)
            st.session_state.sale_preview = None
            refresh()
            st.success("구성을 저장했습니다. 판매 파일을 다시 확인하면 반영됩니다.")
            st.rerun()
...
# 판매구성 관리
# =====================================================================
def page_bom(snap: Snapshot, data: dict, actor: User) -> None:
    store = get_store()
    rows = store.read_boms()
    labels = {c: f"{p['product_name']} {p.get('spec', '')}".strip()
              for c, p in snap.products.items()}
    choices = sorted(labels.values())
    by_label = {v: k for k, v in labels.items()}
    ...
    st.caption(
        "판매처에 올린 상품 하나가 창고의 실물 상품 몇 개로 나뉘는지 적어둔 표입니다. "
        "단품도 여기 들어갑니다. 구성품이 하나뿐인 구성표일 뿐입니다."
    )

    # ---- 조회 ----
    col_a, col_b = st.columns(2)
    channel_filter = col_a.selectbox(
        "판매처", ["전체", "COUPANG", "NAVER"],
        format_func=lambda x: {"전체": "전체", "COUPANG": "쿠팡",
                               "NAVER": "네이버"}.get(x, x))
    keyword = col_b.text_input("검색", placeholder="판매상품명, 옵션, 상품코드")

    grouped: dict[tuple, list[dict]] = {}
    for row in rows:
        if str(row.get("is_active", "TRUE")).upper() == "FALSE":
            continue
        key = (str(row.get("channel", "")), str(row.get("channel_product_id", "")),
               str(row.get("channel_option_id", "")), str(row.get("channel_name", "")),
               str(row.get("option_name", "")))
        grouped.setdefault(key, []).append(row)

    listed = []
    for key, lines in grouped.items():
        channel, product_id, option_id, name, option = key
        if channel_filter != "전체" and channel != channel_filter:
            continue
        composition = " + ".join(
            f"{labels.get(str(l.get('component_product_code')), l.get('component_product_code'))}"
            f" ×{l.get('component_qty')}" for l in lines)
        if keyword:
            hay = f"{name} {option} {product_id} {composition}".lower()
            if keyword.lower() not in hay:
                continue
        listed.append({
            "판매처": "쿠팡" if channel == "COUPANG" else "네이버",
            "판매상품": name or "(이름 없음)",
            "옵션": option,
            "상품번호": product_id,
            "구성": composition,
            "_key": key, "_lines": lines,
        })
    ...
    if listed:
        ...
        st.info("등록된 판매구성이 없습니다. 판매 파일을 올리면 처음 보는 상품부터 물어봅니다.")

    # ---- 수정 ----
    if listed:
        st.markdown("## 구성 수정")
        picked = st.selectbox(
            "대상", list(range(len(listed))),
            format_func=lambda i: f"{listed[i]['판매상품']} / {listed[i]['옵션']}"[:50])
        target = listed[picked]
        channel, product_id, option_id, name, option = target["_key"]

        current = [{"구성품": labels.get(str(l.get("component_product_code")),
                                      str(l.get("component_product_code"))),
                    "개수": int(float(l.get("component_qty") or 1))}
                   for l in target["_lines"]]
        before_text = target["구성"]

        edited = st.data_editor(
            pd.DataFrame(current), hide_index=True, use_container_width=True,
            num_rows="dynamic",
            column_config={
                "구성품": st.column_config.SelectboxColumn(options=choices, width="medium"),
                "개수": st.column_config.NumberColumn(min_value=1, step=1, width="small"),
            },
            key=f"bom_edit_{picked}")
        ...
        col_c, col_d = st.columns(2)
        if col_c.button("구성 저장", type="primary", disabled=not components):
            saving = bom_lib.Bom(
                channel=channel, channel_product_id=product_id,
                channel_option_id=option_id, channel_name=name, option_name=option,
                components=components)
            after_text = bom_lib.describe(saving, labels)
            if after_text == before_text:
                st.info("바뀐 것이 없습니다.")
            else:
                store.deactivate_bom(lambda r: bom_lib.same_target(r, saving))
                store.add_bom_rows(bom_lib.to_rows(saving, actor.login_id), actor.login_id)
                store.append_audit([audit_lib.entry(
                    actor, "BOM_EDIT", feature="판매구성 수정",
                    before=before_text, after=after_text,
                    ref_no=product_id, note=f"{name} / {option}".strip(" /"))])
                refresh()
                st.success("고쳤습니다.")
                st.rerun()

        if col_d.button("이 구성 해제"):
            saving = bom_lib.Bom(
                channel=channel, channel_product_id=product_id,
                channel_option_id=option_id, channel_name=name, option_name=option,
                components=components or [bom_lib.Component("X", 1)])
            store.deactivate_bom(lambda r: bom_lib.same_target(r, saving))
            store.append_audit([audit_lib.entry(
                actor, "BOM_DISABLE", feature="판매구성 해제",
                before=before_text, ref_no=product_id,
                note=f"{name} / {option}".strip(" /"))])
            refresh()
            st.success("해제했습니다. 다음 판매 파일에서 다시 물어봅니다.")
            st.rerun()

        st.caption(
            "구성을 지우지 않고 끕니다. 과거 판매가 어떤 구성으로 차감됐는지 "
            "나중에 되짚을 수 있어야 하기 때문입니다."
        )

    # ---- 새로 만들기 ----
    with st.expander("판매구성 직접 추가"):
        with st.form("new_bom"):
            channel = st.selectbox(
                "판매처", ["COUPANG", "NAVER"],
                format_func=lambda x: "쿠팡" if x == "COUPANG" else "네이버 스마트스토어")
            col_e, col_f = st.columns(2)
            product_id = col_e.text_input("상품번호", placeholder="알면 넣으세요. 가장 정확합니다")
            option_id = col_f.text_input("옵션ID")
            name = st.text_input("판매상품명")
            option = st.text_input("옵션명", placeholder="없으면 비워두세요")
            first = st.selectbox("구성품", choices) if choices else None
            qty = st.number_input("판매 1개당 개수", min_value=1, value=1, step=1)
            st.caption("먼저 한 줄만 만들고, 저장한 뒤 위 '구성 수정'에서 나머지를 더하세요.")

            if st.form_submit_button("만들기"):
                if not (name.strip() or product_id.strip()):
                    st.error("판매상품명이나 상품번호 중 하나는 있어야 합니다.")
                elif not first:
                    st.error("구성품을 골라주세요.")
                else:
                    saving = bom_lib.Bom(
                        channel=channel, channel_product_id=product_id.strip(),
                        channel_option_id=option_id.strip(),
                        channel_name=name.strip(), option_name=option.strip(),
                        components=[bom_lib.Component(by_label[first], int(qty))])
                    store.add_bom_rows(bom_lib.to_rows(saving, actor.login_id), actor.login_id)
                    store.append_audit([audit_lib.entry(
                        actor, "BOM_CREATE", feature="판매구성 등록",
                        after=bom_lib.describe(saving, labels),
                        ref_no=product_id.strip(), note=name.strip())])
                    refresh()
                    st.success("만들었습니다.")
                    st.rerun()
